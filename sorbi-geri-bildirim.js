...
' font-size:.82rem;font-weight:500;color:var(--ink);background:rgba(var(--bg-rgb),.92);',
...
'.gb-kutu{width:100%;max-width:460px;background:var(--bg);border:1px solid rgba(var(--ink-rgb),.12);',
' border-radius:18px;padding:1.2rem 1.15rem 1.1rem;font-family:Inter,system-ui,sans-serif;color:var(--ink);',
' box-shadow:0 24px 60px rgba(var(--bg-rgb),.6);max-height:92vh;overflow-y:auto}',
'.gb-kutu h3{font-family:"Space Grotesk",Inter,sans-serif;font-size:1.05rem;font-weight:600;margin:0 0 .2rem}',
'.gb-kutu p.gb-ac2{font-size:.83rem;color:var(--mut);font-weight:300;margin:0 0 .9rem;line-height:1.55}',
'.gb-tur{display:flex;gap:.35rem;margin-bottom:.7rem;flex-wrap:wrap}',
'.gb-tur button{flex:1 1 auto;font-family:inherit;font-size:.79rem;font-weight:500;color:var(--mut);',
' background:transparent;border:1px solid rgba(var(--ink-rgb),.12);border-radius:10px;padding:.42rem .6rem;cursor:pointer;transition:.15s}',
'.gb-tur button.on{color:var(--bg);background:var(--acc);border-color:rgba(var(--ink-rgb),.14)}',
'.gb-kutu textarea,.gb-kutu input{width:100%;font-family:inherit;font-size:.88rem;color:var(--ink);',
' background:rgba(var(--bg-rgb),.34);border:1px solid rgba(var(--ink-rgb),.12);border-radius:11px;',
...
'.gb-kutu textarea:focus,.gb-kutu input:focus{border-color:rgba(var(--ink-rgb),.55)}',
'.gb-kutu ::placeholder{color:var(--dim)}',
'.gb-alt{display:flex;gap:.5rem;align-items:center;margin-top:.7rem;flex-wrap:wrap}',
'.gb-gonder{font-family:inherit;font-size:.86rem;font-weight:600;color:var(--bg);background:var(--acc);',
' border:none;border-radius:11px;padding:.55rem 1.1rem;cursor:pointer}',
'.gb-gonder:hover{background:var(--acc)}.gb-gonder:disabled{opacity:.5;cursor:default}',
'.gb-vazgec{font-family:inherit;font-size:.82rem;color:var(--mut);background:none;border:none;cursor:pointer;padding:.5rem}',
'.gb-vazgec:hover{color:var(--ink)}',
'.gb-st{font-size:.8rem;color:var(--dim);min-height:1.1em;margin-left:auto}',
'.gb-st.iyi{color:var(--mut)}.gb-st.hata{color:var(--acc)}',
