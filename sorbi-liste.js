...
'.sbl .sbl-f{display:flex;gap:.5rem;flex-wrap:wrap;justify-content:center;max-width:430px;margin:0 auto}'+
...
'.sbl label.sbl-ok{display:flex!important;gap:.5rem;align-items:flex-start;justify-content:center;text-align:left!important;font-size:.78rem!important;color:#9BA0AB!important;line-height:1.5!important;max-width:430px;margin:.8rem auto 0!important;text-transform:none!important;letter-spacing:0!important;font-weight:400!important}'+
...
function kur(el){
  ...
  el.innerHTML='<h3>Sorbi telefonuna geliyor</h3>'+
   ...
   '<div class="sbl-f"><input type="email" placeholder="e-posta adresin" autocomplete="email" aria-label="E-posta adresin">'+
   '<button type="button">Beni listeye yaz ✦</button>'+
  ...
  var em=el.querySelector('input[type=email]'), btn=el.querySelector('button'), hp=el.querySelector('input[type=text]'),
      ok=el.querySelector('label input'), st=el.querySelector('.sbl-st');
  try{ if(localStorage.getItem('sorbi_wl')==='1'){ el.querySelector('.sbl-f').style.display='none'; el.querySelector('label').style.display='none'; st.style.color='#E4CF9A'; st.textContent='Listedesin — çıkınca ilk sana yazacağız.'; } }catch(e){}
